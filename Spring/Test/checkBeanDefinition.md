...
![](/resources/imgs/checkBeanDefinition_1.JPG)
...
![](/resources/imgs/checkBeanDefinition_2.JPG)

AppConfig 클래스에서 등록한 빈만 선택적으로 확인할 수 있다.

반대로, 스프링Container 내에 기본으로 등록된 bean만 확인하고자 할 때는 조건식에서 

`BeanDefinition.ROLE_APPLICATION` 을 `BeanDefinition.ROLE_INFRASTRUCTURE` 으로 변경해주면 된다.
...
public class ApplicationContextExtendsFindTest {
    ...
    void findAllBeanByParentType() {
        ...
        assertThat(beansOfType.size()).isEqualTo(2);
        // 테스트 케이스 작성시, 통과 실패는 시스템이 결정하게 해야함(눈으로 보고 있을 수 없음).
        // `sout` 으로 출력해보는 것은 학습용이 아닌 실무에서는 크게 의미가 없음.
        for (String key : beansOfType.keySet()) {
            System.out.println("key = " + key + " value = " + beansOfType.get(key));
        }
    }
